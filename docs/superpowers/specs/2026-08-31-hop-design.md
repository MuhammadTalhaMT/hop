...
1. Cursor crosses the top edge onto the PC and returns via the bottom
2. Typing lands on the PC, and Cmd+C on the Mac keyboard copies on the PC
3. Lock the Mac for ten minutes, unlock, confirm sharing resumes with no
   manual restart
4. Sleep the Mac fully, wake it, confirm the same
5. Reboot the PC, confirm the client reconnects on its own
6. Panic hotkey returns control to the Mac
